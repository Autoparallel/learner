...
fn test_doi_config_deserialization() {
  let config_str =
    read_to_string("config/retrievers/doi.toml").expect("Failed to read config file");
...
fn test_iacr_config_deserialization() {
  let config_str =
    read_to_string("config/retrievers/iacr.toml").expect("Failed to read config file");
