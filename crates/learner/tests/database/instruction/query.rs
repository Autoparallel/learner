...
async fn test_search_ordering() -> TestResult<()> {
  ...
  let results = Query::text("paper").order_by(OrderField::PublicationDate).execute(&mut db).await?;
...
async fn test_combined_queries_ordering() -> TestResult<()> {
  ...
  let results =
    Query::text("test").order_by(OrderField::PublicationDate).descending().execute(&mut db).await?;
