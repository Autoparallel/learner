...
// // TODO: Note there are more things we get in a typical response which are probably useful
// honestly. // I think we should capture those and also potentially put all of this in the `Source`
// enum // variants so that the `Paper` struct contains all the relevant metadata.
